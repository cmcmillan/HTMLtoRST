...
def makeWatermarkPdf(pdfName, classification = "UNCLASSIFIED", title = None, 
    header = True, footer = True, fontName = "Times-Bold", fontSize = 6, pageSize = letter):
    
    watermark = Canvas(pdfName, pagesize = pageSize)
    makeWatermark( watermark, classification, title, header, footer, fontName, fontSize)
    watermark.save()

def main():
    
    makeWatermarkPdf("watermark.pdf", title = "The Business Data Catalog", pageSize = letter)
